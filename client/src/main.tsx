    ...
    return (
        <main className="landing">
            <div className="brand-mark">
                <Zap size={18} fill="currentColor" /> TECHNICAL AWARENESS
            </div>
            <div className="hero-grid">
                <section>
                    <p className="eyebrow">LIVE CLASSROOM QUIZ PLATFORM</p>
                    <h1>
                        Make every answer
                        <br />
                        <em>count.</em>
...
function Join() {
    ...
    useEffect(() => {
        if (!joined || !state?.gameCode) return;
        const s = connect();
        const rejoin = () =>
            s.emit(
                "join_game",
...
function FinalAnalysis({
    ...
    selectParticipant: (leader: Leader) => void;
    sessionId: string;
}) {
    return (
        <main className="app-shell host-shell">
            <section className="host-content">
                <p className="eyebrow">QUIZ ANALYSIS</p>
...
function Host() {
    ...
    useEffect(() => {
        ...
        const joinUrl = `${window.location.origin}/join?code=${encodeURIComponent(session.gameCode)}`;
    ...
    useEffect(() => {
        if (!ended || !session?.id) return;
        fetch(`${API}/api/sessions/${session.id}/analytics`)
            ...
            <main className="app-shell centered">
                ...
                <section className="host-start">
                    ...
                    <button className="primary full" onClick={create}>
                        Create live session <ArrowRight size={17} />
                    </button>
                </section>
            </main>
        );
    ...
    const selectParticipant = async (leader: Leader) => {
        const response = await fetch(
            `${API}/api/sessions/${session.id}/participants/${leader.id}/results`,
        );
        if (response.ok) setSelectedParticipant(await response.json());
    };
    if (ended && !showAnalytics)
        return (
            <main className="app-shell host-shell">
                <section className="results-wrap">
                    <p className="eyebrow">FINAL QUIZ RESULTS</p>
                    <h1>Podium</h1>
                    ...
                    >
                        View quiz analysis <ArrowRight size={17} />
                    </button>
                </section>
            </main>
        );
    ...
    if (question && questionStats && leaderboardOpen)
        return (
            <main className="app-shell host-shell">
                <header className="topbar">
                    <a className="brand" href="/">
            ...
            <main className="app-shell host-shell">
                ...
                <section className="host-content">
                    ...
                    <div className="distribution">
                        {questionStats.distribution?.map((item: any) => (
                            <div className="distribution-row" key={item.key}>
                                <b>{item.key}</b>
                                <span>
                                    <i
                                        style={{
                                            width: `${questionStats.answered ? (item.count / questionStats.answered) * 100 : 0}%`,
                                        }}
                                    />
                                </span>
                    ...
                    <div className="host-actions">
                        <button className="primary" onClick={viewLeaderboard}>
            ...
            <section className="host-content">
                <div className="host-header">
                    ...
                    <div className="code-block">
                        ...
                        <button onClick={copy} title="Copy game code">
                ...
                {ended ? (
                    <>
                        ...
                        <a
                            className="secondary export"
                            href={`${API}/api/sessions/${session.id}/export`}
                        >
                            Download CSV
                        </a>
                    </>
                    ...
                    <div className="lobby">
                        <div className="lobby-code">
                            ...
                            <strong>/join</strong>
                            <p>Share the game code above with your class.</p>
                        </div>
